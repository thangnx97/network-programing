...
*/
int getfilePathAndKeyForDecode(char * filePath, unsigned char *key) {
	int done = 0;
	do {
		printf("Enter file patch\n");
		fflush(stdin);
...
int decodeProcess(SOCKET connSock, char *buff, int buffSize) {
	...
	short *pLength = (short*)&buff[1];
	unsigned char key;
	ret = getfilePathAndKeyForDecode(filePath, &key);
	if (ret == 0) return ret;
